...
function time2str(date) {
    ...
    time = time / 60;
    if (time < 24) {
        return parseInt(time) + ' Hour ago'
    }
    time = time / 24;
    if (time < 7) {
        return parseIn(time) + 'days ago'
    }
        ...
        success: function (response) {
            if (response['result'] === 'success') {
                ...
                for (let i = 0; i < posts.length; i++) {
                    ...
                    let class_star = post['star_by_me'] ? 'fa fa-star' : 'fa fa-star-o';
                    let class_thumbsup = post['thumbsup_by_me'] ? 'fa fa-thumbs-up' : 'fa fa-thumbs-o-up';

                    let html_temp = `
                    <div class="box">
        <article class="media" id="${post['_id']}">
            ...
            <div class="media-content">
                ...
                <nav class="level-mobile">
                    <div class="level-left">
                        <a class="level-item is-sparta" aria-label="heart" onclick="toggle_like('${post["_id"]}', 'heart')">
                            <span class="icon is-small">
                                <i class="${class_heart}" aria-hidden="true"></i>
                        ...
                        <a class="level-item is-sparta" aria-label="star" onclick="toggle_star('${post["_id"]}', 'star')">
                            <span class="icon is-small">
                                <i class="${class_star}" aria-hidden="true"></i>
                        ...
                        <a class="level-item is-sparta" aria-label="thumbsup" onclick="toggle_thumbsup('${post["_id"]}', 'thumbsup')">
                            <span class="icon is-small">
                                <i class="${class_thumbsup}" aria-hidden="true"></i>
